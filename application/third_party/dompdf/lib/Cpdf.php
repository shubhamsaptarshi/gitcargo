...
EOT;
                    ...
                    if ($options['type'] === 'png') {
                        ...
                        if ($options['isMask']) {
                            ...
                            if (mb_strlen($options['pdata'], '8bit')) {
                                ...
                                $info['ColorSpace'] = DIRECTORY_SEPARATOR . $options['color'];
                            }
                        }
    ...
    private function openFont($font)
    {
        ...
        $name = basename($font);
        $dir = dirname($font) . DIRECTORY_SEPARATOR;
        ...
        if (file_exists($fontcache . DIRECTORY_SEPARATOR . $cache_name)) {
            $this->addMessage("openFont: php file exists $fontcache/$cache_name");
            $this->fonts[$font] = require($fontcache . DIRECTORY_SEPARATOR . $cache_name);
            ...
            if (file_exists($fontcache . DIRECTORY_SEPARATOR . $old_cache_name)) {
                $this->addMessage(
                    "openFont: php file doesn't exist $fontcache/$cache_name, creating it from the old format"
                );
                $old_cache = file_get_contents($fontcache . DIRECTORY_SEPARATOR . $old_cache_name);
                file_put_contents($fontcache . DIRECTORY_SEPARATOR . $cache_name, '<?php return ' . $old_cache . ';');
        ...
        if (!isset($this->fonts[$font]) && file_exists($dir . $metrics_name)) {
            ...
            if (is_dir($fontcache) && is_writable($fontcache)) {
                file_put_contents($fontcache . DIRECTORY_SEPARATOR . $cache_name, '<?php return ' . var_export($data, true) . ';');
            }
            $data = null;
        }
    ...
    function selectFont($fontName, $encoding = '', $set = true)
    {
        ...
        if (!isset($this->fonts[$fontName])) {
            ...
            if (isset($this->fonts[$fontName])) {
                ...
                $dir = dirname($fontName) . DIRECTORY_SEPARATOR;
...
//        }
     ...
     * @return string
     */
    function filterText($text, $bom = false, $convert_encoding = true)
    {
        if (!$this->numFonts) {
            $this->selectFont($this->defaultFont);
        }
