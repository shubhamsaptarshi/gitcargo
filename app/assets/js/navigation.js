...
(function ($, window, document, undefined) {
    $.navigation = function (element, options) {
        ...
        if ($(element).find(".nav-search").length > 0) {
            $(element).find(".nav-search").find("form").prepend("<span class='nav-search-close-button'>&#10005;</span>");
        }
        plugin.init = function () {
            ...
            if (plugin.settings.hidden) {
                ...
                plugin.settings.tabBreakpoint = 99999;
            }
            checkSubmenus();
            ...
            $(element).find(".nav-menu li a").on("click touchstart", function (e) {
                // e.stopPropagation(); 
                // e.preventDefault();
        ...
        plugin.showSubmenu = function (parentItem, submenuEffect) {
            if (windowWidth() > plugin.settings.tabBreakpoint) {
        ...
        var initNavigationMode = function () {
            if (plugin.settings.responsive) {
                if (windowWidth() <= plugin.settings.tabBreakpoint && bigScreenFlag > plugin.settings.tabBreakpoint) {
                    $(element).addClass("navigation-portrait").removeClass("navigation-landscape");
                    portraitMode();
                }
                if (windowWidth() > plugin.settings.tabBreakpoint && smallScreenFlag <= plugin.settings.tabBreakpoint) {
        ...
        var goOut = function () {
            $("body").on("click.body touchstart.body", function (e) {
                if ($(e.target).closest(".navigation").length === 0) {
                    ...
                    $(element).find(".nav-submenu-open").removeClass("nav-submenu-open");
                    $(element).find(".nav-search").find("form").slideUp();
                }
            });
        };
